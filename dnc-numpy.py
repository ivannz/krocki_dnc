...
def train(inputs, targets, cprev, hprev, mprev, rprev):
    ...
    for t, input in enumerate(inputs):
        ...
        ###########################
        ps[t] = np.exp(ys[t] - logsumexp(ys[t], axis=0))  # probabilities for next chars
    ...
    for t in reversed(range(len(inputs))):
        dy = np.copy(ps[t])
        for b in range(B):
            dy[targets[t][b], b] -= 1 # backprop into y
        ...
        #iface gates
        dmem_write_gate = np.dot(
            W_ones.T,
            dmemory * (mem_new_content[t] - mem_erase_gate[t] * memory[t-1])
        ) # 1
...
while t < T:
    ...
    for b in range(B): p[b] += S # move data pointer
